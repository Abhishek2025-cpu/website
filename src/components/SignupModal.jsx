...
const API_BASE_URL = 'https://kalpyotish.onrender.com/api/auth';
...
const SignupModal = ({ isOpen, onClose }) => {
  ...
  const [formData, setFormData] = useState({ name: '', gender: '', city: '', mobileNo: '', profile: null, dateOfBirth: null, timeOfBirth: '' });
  ...
  const resetState = () => { setStep(1); setAuthMode('signup'); setEmail(''); setPassword(''); setOtp(''); setFormData({ name: '', gender: '', city: '', mobileNo: '', profile: null, dateOfBirth: null, timeOfBirth: '' }); setProfilePreview(null); setError(''); setIsLoading(false); onClose(); };
  const changeStep = (newStep, newDirection = 1) => { setDirection(newDirection); setStep(newStep); };
  const handleSendOtp = async (e) => { e.preventDefault(); setError(''); setIsLoading(true); try { await axios.post(`${API_BASE_URL}/send-otp`, { email }); changeStep(2); } catch (err) { setError(err.response?.data?.message || 'Failed to send OTP.'); } finally { setIsLoading(false); } };
  const handleVerifyOtp = async (e) => { e.preventDefault(); setError(''); setIsLoading(true); try { await axios.post(`${API_BASE_URL}/verify-otp`, { email, otp }); changeStep(3); } catch (err) { setError(err.response?.data?.message || 'Invalid OTP.'); } finally { setIsLoading(false); } };
  const handleLogin = async (e) => { e.preventDefault(); setError(''); setIsLoading(true); try { console.log("Login successful (API call is a placeholder)"); changeStep(4); setTimeout(resetState, 2000); } catch (err) { setError('Login failed (API is placeholder).'); } finally { setIsLoading(false); } };
  const handleSignup = async (e) => { e.preventDefault(); setError(''); setIsLoading(true); const submissionData = new FormData(); const formattedData = { ...formData, dateOfBirth: formData.dateOfBirth ? formData.dateOfBirth.toISOString().split('T')[0] : '', }; submissionData.append('email', email); for (const key in formattedData) { if(key !== 'profile') submissionData.append(key, formattedData[key]); } if (formData.profile) submissionData.append('profile', formData.profile); try { const res = await axios.post(`${API_BASE_URL}/signup`, submissionData, { headers: { 'Content-Type': 'multipart/form-data' } }); const { token } = res.data; const expiryTime = new Date().getTime() + 2 * 60 * 60 * 1000; localStorage.setItem('authToken', token); localStorage.setItem('tokenExpiry', expiryTime); changeStep(4); setTimeout(resetState, 2000); } catch (err) { setError(err.response?.data?.message || 'Signup failed.'); } finally { setIsLoading(false); } };
