...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    fetch(`https://kalpjyotish.com/api/api/astrologer/all`)
      .then((res) => res.json())
      .then((data) => {
        const astrologer = data.data.find(astro => astro._id === userId);
        if (astrologer) {
          setProfileData(astrologer);
        } else {
  ...
  if (error || !profileData) {
    return <div className="profile-container"><div className="profile-message">{error || "Could not load profile."}</div></div>;
  }
